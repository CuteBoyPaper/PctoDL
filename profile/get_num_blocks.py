...
parser.add_argument('--max_threads_sm', type=int, default=1536,
...
for index, row in df.iterrows():
    num_blocks = int(re.sub(r'[^\d.]', '', row['Grid']))
    num_threads = row['Number_of_threads']
    if isinstance(row['Block'],int):
        threads_per_block = row['Block']
    else:
        threads_per_block = int(re.sub(r'[^\d.]', '', row['Block']))
    shmem_per_block = int(re.sub(r'[^\d.]', '', row['Static_shmem_per_block']))
