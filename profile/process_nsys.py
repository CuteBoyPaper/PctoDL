...
l2_cache = list(df_ncu['L2 Cache Throughput'])
...
for j in range(len(names)):
    if 'memcpy' not in names[j] and 'memset' not in names[j]:
        ...
        l2_cache_new.append(l2_cache[i])
...
if plot_mem:
    ...
    all = 0
    for i in range(len(start_time)-1):
        current += l2_cache[i] * (start_time[i+1]-start_time[i])
        all += 100 * (start_time[i+1]-start_time[i])
    avg = current*100/all
    print("current is: ", current)
    print("overall is: ", all)

    print(f"average l2 cache util is {avg} %")

    plt.stairs(l2_cache, start_time, linewidth=1.5) #, marker="o")
    ax1.set_xlabel('Time (sec)', fontsize=22)
    ax1.set_ylabel('L2 Cache Throughput (%)', fontsize=22)
    ax1.set_ylim(0,102)
    ...
    plt.savefig(f'{pwd}/l2cache_over_time.png', bbox_inches="tight")
