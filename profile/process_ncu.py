...
kernels = []
metrics_to_get = ['Duration', 'Block Size', 'Grid Size','L2 Cache Throughput', 'Compute (SM) Throughput', 'DRAM Throughput', 'Registers Per Thread', 'Static Shared Memory Per Block']
...
for kernel in kernels:
    num_threads = int(re.sub(r'[^\d.]', '', kernel[-2])) * int(re.sub(r'[^\d.]', '', kernel[-3]))
    num_registers = num_threads * int(re.sub(r'[^\d.]', '', kernel[-1]))
    kernel += [num_threads, num_registers]

print(len(kernels))
# print(kernels[0])
labels = ['Kernel_Name', 'DRAM_Throughput(%)', 'Duration(ns)','L2 Cache Throughput', 'Compute(SM)(%)',  'Block', 'Grid', 'Registers_Per_Thread', 'Static_shmem_per_block', 'Number_of_threads', 'Number_of_registers']
